...
public class GoogleSqlDriverUIContributer implements IDriverUIContributor,
  ...
  public void loadProperties() {
    ...
    String instanceName = this.properties.getProperty(GOOGLESQL_INSTANCENAME_PROP_ID);
  ...
  public void setProperties() {
    ...
    properties.setProperty(GOOGLESQL_INSTANCENAME_PROP_ID,
        this.instanceNameText.getText());
    setJDBCUrl(properties);
  ...
  // Get the part of the jdbc url string after "?". This forms the argument
  // section
  // of the url.
  private String getJDBCArguments(String url) {
    if (url == null) {
      return null;
    }
  ...
  private void setJDBCUrl(Properties prop) {
    String oldUrl = prop.getProperty(IJDBCConnectionProfileConstants.URL_PROP_ID);
    String arg = getJDBCArguments(oldUrl);
    if (arg == null) {
      arg = "";
    } else if (arg.isEmpty() == false) {
      arg = "?" + arg;
    }
    String url = "jdbc:google:rdbms://" + instanceNameText.getText() + "/"
        + databaseText.getText() + arg;
    prop.setProperty(IJDBCConnectionProfileConstants.URL_PROP_ID, url);
  }
}
