...
public class ManagedApiPlugin extends AbstractGooglePlugin {
  ...
  public static final String COPY_CLASSPATH_ENTRIES_TARGET_PATH_KEY = "COPY_CLASSPATH_ENTRIES_TARGET_PATH";

  public static final String DEFAULT_MANAGED_API_DIRECTORY_HREF = "http://api-directory.googleapis.com/5935";

  public static final String DEFAULT_MANAGED_API_ICON_PREFETCH_HREF = "http://api-directory.googleapis.com/icons";
  ...
  public static final String MANAGED_API_ROOT_FOLDER_DEFAULT_PATH = ".google_apis";

  public static final String MANAGED_API_ROOT_FOLDER_PATH_KEY = "MANAGED_API_ROOT_FOLDER_PATH";

  public static final QualifiedName MANAGED_API_SESSION_KEY_QNAME = new QualifiedName(
  ...
  public static boolean isValidToAddManagedApiProjectState(IProject project) {
    try {
      if (NatureUtils.hasNature(project, JavaCore.NATURE_ID)) {
        ...
        getDefault().extensionsLock.lock();
        try {
          for (IManagedApiProjectStateTest tester : getDefault().managedApiProjectStateTests) {
   ...
   * extensionsLock.
   */
  private IManagedApiProjectInitializationCallback[] projectInitializationCallbacks = null;
  ...
  private void loadExtensions() {
    ...
    List<IManagedApiProjectStateTest> tests = new ArrayList<IManagedApiProjectStateTest>();

    for (ExtensionQuery.Data<IManagedApiProjectStateTest> testExtension : testExtensions) {
      if (testExtension.getExtensionPointData() != null) {
        tests.add(testExtension.getExtensionPointData());
      }
    }

    ExtensionQuery<IManagedApiProjectInitializationCallback> projectInitCallbackExtQuery = new ExtensionQuery<IManagedApiProjectInitializationCallback>(
        ManagedApiPlugin.PLUGIN_ID, "managedApiProjectInitializationCallback",
        "class");
    List<ExtensionQuery.Data<IManagedApiProjectInitializationCallback>> projectInitCallbackExtensions = projectInitCallbackExtQuery.getData();
    List<IManagedApiProjectInitializationCallback> initCallbacks = new ArrayList<IManagedApiProjectInitializationCallback>();

    for (ExtensionQuery.Data<IManagedApiProjectInitializationCallback> testCallback : projectInitCallbackExtensions) {
    ...
    try {
      extensionsLock.lock();
      managedApiProjectStateTests = tests.toArray(new IManagedApiProjectStateTest[tests.size()]);
      projectInitializationCallbacks = initCallbacks.toArray(new IManagedApiProjectInitializationCallback[initCallbacks.size()]);
    } finally {
