...
public class SqlUtilities {
  ...
  private static final String CLOUDSQL_URL_VALUE_FORMAT = "jdbc:google:rdbms://%s/%s?";
  private static final String CLOUDSQL_URL_VALUE_OAUTH2_FORMAT = "oauth2RefreshToken=%s&";
  private static final String CLOUDSQL_URL_VALUE_CLIENTID_FORMAT = "oauth2ClientId=%s&";
  private static final String CLOUDSQL_URL_VALUE_CLIENTSECRET_FORMAT = "oauth2ClientSecret=%s&";
   ...
   * @return cloud sql jdbc url.
   */
  public static String getCloudSqlJdbcUrl(String instance, String database,
      boolean withAuthorizationArguments) {
    if (instance.trim().isEmpty() || database.trim().isEmpty()) {
      return "";
    }
    ...
    if (withAuthorizationArguments) {
      ...
      if (!refreshToken.isEmpty()) {
        rdbmsUrlValue.append(String.format(CLOUDSQL_URL_VALUE_OAUTH2_FORMAT, refreshToken));
      }
      if (!clientID.isEmpty()) {
        rdbmsUrlValue.append(String.format(CLOUDSQL_URL_VALUE_CLIENTID_FORMAT, clientID));
      }
      if (!clientSecret.isEmpty()) {
        rdbmsUrlValue.append(String.format(CLOUDSQL_URL_VALUE_CLIENTSECRET_FORMAT, clientSecret));
      }
    }
   ...
   * @return mysql jdbc url.
   */
  public static String getMySqlJdbcUrl(String host, String database, String port,
      String user, String password) {
    if (host.trim().isEmpty() || database.trim().isEmpty() || port.trim().isEmpty()) {
      return "";
    }
    ...
    if (!password.isEmpty()) {
      rdbmsUrlValue.append(String.format(MYSQL_URL_VALUE_PASSWORD_FORMAT, password));
    }
    // The last char will be either ? or & which is not required.
    rdbmsUrlValue.deleteCharAt(rdbmsUrlValue.length() - 1);
    return rdbmsUrlValue.toString();
  ...
  public static String getMySqlUrl(IProject project) {
    ...
    String host = GoogleCloudSqlProperties.getMySqlHostName(project);
    String port = new Integer(GoogleCloudSqlProperties.getMySqlPort(project)).toString();
